    ...
    if ($("#teacher-chart").length) {
      ...
      var teacherChartOptions = {
        ...
        legendCallback: function(chart) {
          var text = [];
          text.push('<ul>');
          for (var i = 0; i < teacherChartData.datasets[0].data.length; i++) {
              text.push('<li><span class="legend-dots" style="background:' +
              teacherChartData.datasets[0].legendColor[i] +
                          '"></span>');
              if (teacherChartData.labels[i]) {
                  text.push(teacherChartData.labels[i]);
              }
              text.push('<span class="float-right">'+teacherChartData.datasets[0].data[i]+"%"+'</span>')
              text.push('</li>');
          }
          text.push('</ul>');
          return text.join('');
      ...
      var teacherChartCanvas = $("#teacher-chart").get(0).getContext("2d");
      var teacherChart = new Chart(teacherChartCanvas, {
        type: 'doughnut',
        data: teacherChartData,
        options: teacherChartOptions
      });
      $("#teacher-chart-legend").html(teacherChart.generateLegend());
    }
